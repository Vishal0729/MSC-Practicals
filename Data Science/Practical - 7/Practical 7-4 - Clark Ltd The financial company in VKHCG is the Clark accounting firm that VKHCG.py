...
if sys.platform == 'linux': 
    ...
    Base=r"D:/MSC Practicals/Data Science/Practical - 7/"
print('################################')
print('Working Base :',Base, ' using ', sys.platform)
print('################################')
sInputTemplateName='Balance-Sheet-Template.xlsx'
sOutputFileName='Report-Balance-Sheet'
Company='04-Clark'
sDatabaseName=Base + '/Inputs/' + Company + '/clark.db'
conn = sq.connect(sDatabaseName)
#conn = sq.connect(':memory:')
### Import Balance Sheet Data
for y in range(1,13):
    sInputFileName='BalanceSheets' + str(y).zfill(2) + '.csv'
    sFileName=Base + '/Inputs/' + Company + '/' + sInputFileName
    print('################################')
...
for i in range(sDates.shape[0]):
    ...
    sFileName=Base + '/Outputs/' + Company + '/' + sOutputFileName + \
    '-' + sCompanyFile + '.xlsx'
    print(sFileName)
